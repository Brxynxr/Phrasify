...
function App() {
  ...
  const alHacerClickComenzar = () => {
    const elementoDestino = document.getElementById('portal-app-container');
    if (elementoDestino) {
  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col font-sans relative bg-transparent">
      ...
      {/* Fondo Gótico en la capa más profunda */}
      <div className="fixed inset-0 goth-bg-main -z-20 pointer-events-none" />

      {/* Canvas Partículas de Fondo Global en la capa intermedia */}
      <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none -z-10 opacity-90" />

      {/* Landing de presentación inicial */}
      <Landing alHacerClickComenzar={alHacerClickComenzar} />

      {/* Contenedor de la Aplicación (Módulos y navegación) */}
      <div id="portal-app-container" className="flex flex-col min-h-screen">
